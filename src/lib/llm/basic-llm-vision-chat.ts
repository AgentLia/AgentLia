...
export class BaseVision {
  ...
  extractImageTag(content: string) {
  ...
  async call(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const latestMessage = options.history
          ?.filter((o) => o instanceof AIMessage)
          .filter((o) => o.content)
          .reverse()
          .pop();
        const isLatestMessageImg = (latestMessage?.content as string)?.includes(
          "<img"
        );
        ...
        const imageTags = this.extractImageTag(
          (latestMessage?.content as string) ?? ""
        );
        if (imageTags && imageTags?.length > 0) {
          const src = imageTags[0].match(/src="([^"]*)"/)[1];
          ...
          if (isLatestMessageImg) {
            ...
            let imageSrc: any = (latestMessage?.content as string).match(
              /<img[^>]+src="([^">]+)"/
            );
        ...
        let history = options.history?.slice(
          options?.history?.findIndex((o) => o.content == undefined) + 1
        )?.reverse();
