...
async function langchainRag(
  ...
  const hasMatch = await checkTextsMatchLLM(
    `ID:${metaData?.id}. ${firstDocContent}. `,
    query,
    model
  );
  ...
  const questionPrompt = new PromptTemplate({
    ...
    template: strict
      ? questionPromptTemplateStringRag
      : questionPromptTemplateString,
  });
  ...
  try {
    ...
    // response.sourceDocuments = sourceDocuments; // TODO cluster the source documents
