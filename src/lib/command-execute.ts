...
import { getConvoId, getSimilarity, hasFiles } from "./utils";
import {
...
async function getIntentCommandByPrompt(prompt: string) {
  const taskCmds = appState.commands.filter((o) => o.isTool);
  if (taskCmds.length == 0) {
    return undefined;
  }

  const promises = taskCmds.map((checksum) =>
    getSimilarity(prompt, checksum.description)
  );
  const results: number[] = await Promise.all(promises);

  const indexBest = results.indexOf(Math.max(...results));
  const bestCmd = taskCmds[indexBest];

  if(results[indexBest] > 0.3) {
    return bestCmd;
  }
...
export async function execCommands(
  ...
  convoId?: boolean | string,
  killStream = false,
  initalDocs?: DocumentData[]
) {
  ...
  try {
    ...
    for (let command of commands) {
      try {
        const result = await execActions(
          prompt,
          initalPrompt,
          command,
          initalDocs
...
export async function execAction(
  ...
  if (typeof action.type != "string") {
    ...
    let params: any = { input, actionState };
  ...
  switch (action.type) {
    ...
    case "DefaultLLM":
      return setDefaultLlm(
        input,
        action as VectorRequestActionModel,
        actionState
      );
    case "CallJsScript":
      return await callJsScript(
        input,
        action as JsScripActionModel,
        actionState
      );
  }
}

function setDefaultLlm(
  input: string,
  action: DefaultLLMActionModel,
...
async function callRagLlm(
  ...
  const llmSetting = appState.keyValues?.settings?.find(
    (e) => e.id === action.endpointId
  );
  ...
  let result = await LlmApi.reqRag(
    prompt,
    llmSetting,
    state.docs,
    hasFiles(),
...
async function callVector(
  input: string,
  action: VectorRequestActionModel,
  actionstate: ActionState
) {
  let prompt = input ?? "";
...
async function callJsScript(
  input: string,
  action: JsScripActionModel,
  actionstate: ActionState
) {
